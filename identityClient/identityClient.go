...
func (c *E3dbIdentityClient) UpdateIdentityAttributes(ctx context.Context, params UpdateIdentityAttributesRequest) error {
	path := c.Host + identityServiceBasePath + "/admin/" + realmResourceName + "/" + strings.ToLower(params.RealmName) + "/" + identityResourceName + "/" + params.ToznyID.String() + "/attributes"
	req, err := e3dbClients.CreateRequest(http.MethodPut, path, params)
	if err != nil {
		return err
	}
...
// Get Realm's storage limit based on the key "storage limit"
func (c *E3dbIdentityClient) GetRealmAttributeStorageLimit(ctx context.Context, realmName string) (*RealmAttribute, error) {
